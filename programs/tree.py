...
class Tree(object):
    def __init__(
        ...
        features = "{} {} {} {}".format(
            ccrFeature if ccrFeature is not None else "",
            ptFeature if ptFeature is not None else "",
            posFeature if posFeature is not None else "",
            motherFeature if motherFeature is not None else "",
        )
        ...
        if otypes is None:
            otypes = [x[0] for x in C.levels.data]
        ...
        info(
            "Start computing parent and children relations for objects of type {}".format(
                ", ".join(otypes)
            )
        )
        otypeSet = set(otypes)
        ...
        for node in N.walk():
            ...
            for si in range(ls):
                ...
                if set(nmSlots) <= set(smSlots):
                    ...
                    break
            curStack = [
                curStack[i] for i in range(len(curStack)) if i not in tobeRemoved
            ]
            if otype != baseType:
                curStack.append((node, nmSlots, nmMax))
        info(
            "{} nodes: {} have parents and {} have children".format(
                nn, len(eparent), len(echildren)
            )
        )
    ...
    def restructureClauses(self, ccrClass):
        ...
        if mo:
            error(
                "{} {}s have mothers of types outside {}.\nThese mother relationships will be ignored".format(
                    mo, self.clauseType, otypeSet
                )
            )
            for mt in sorted(moutside):
                error(
                    "{} mothers point to {} nodes".format(moutside[mt], mt),
                    withtime=False,
                )
        else:
            info(
                "All {}s have mothers of types in {}".format(self.clauseType, otypeSet)
            )
        ...
        for cnode in F.otype.s(self.clauseType):
            ...
            if cclass == "n" or cclass == "x":
                ...
                if mnode not in rparent:
                    error(
                        "Should not happen: node without parent: [{} {}]({}) =mother=> [{} {}]({}) =/=> parent".format(
                            self.clauseType,
                            Eoslotss(cnode),
                            cnode,
                            mtype,
        ...
        info(
            "Mothers applied. Found {} motherless {}s.".format(
                len(motherless), self.clauseType
            )
        )
        ts = 0
        for lt in sorted(sisterCount):
            c = sisterCount[lt]
            ts += c * lt
            info("{} nodes have {} sisters".format(c, lt))
    ...
    def getSisters(self, node):
        sisters = self.sisters
        sortKey = self.api.N.sortKey

        def _getSisters(n):
            result = (n,)
            if n in sisters and len(sisters[n]):
                for sn in sisters[n]:
                    result += _getSisters(sn)
            return result
        ...
        def _debugWriteTree(node, level, indent):
            ...
            result.append(
                "{:>2}{:<30} {:<30}] ({:>3}) {:<8} <{}>\n".format(
                    level,
                    "{}[{:<10}".format(
                        indent, "{}{}{}".format(otype, subTypeSep, subType)
                    ),
                    self.slotss(node),
                    rep(node),
                    mspec,
    ...
    def writeTree(self, node, kind, getTag, rev=False, leafNumbers=True):
        ...
        def _writeTree(node):
                    ...
                    if len(snodes) == 1:
                        ...
                        for snode in snodes:
                            if snode == cnode:
                                ...
                                for csnode in self.getChildren(snode, kind):
                                    _writeTree(csnode)
            ...
            if not isWord:
                sequential.append(("C", tag))

        def doSequential():
            if leafNumbers:
                wordRep = " ".join(x[1] for x in sorted(words, key=lambda x: x[0]))
            else:
                wordRep = " ".join(str(x[0]) for x in words)

            treeRep = []
            for (code, data) in sequential:
                if code == "O" or code == "C":
                    if code == "O":
                        treeRep.append("({}".format(data))
                    else:
                        treeRep.append(")")
                elif code == "W":
            ...
            return (
                "".join(treeRep),
                wordRep[::-1] if rev and leafNumbers else wordRep,
                bSlot,
            )
    ...
    def depth(self, node, kind):
        ...
        def _depth(node, kind):
            ...
            cdepth = (
                1 + max(_depth(c, kind) for c in children[node]) if hasChildren else 0
            )
            sdepth = (
                1 + max(_depth(s, kind) for s in sisters[node]) if hasSisters else 0
            )
            if kind == "e":
                return cdepth
            elif kind == "r":
                return max(cdepth + 1, sdepth) if hasSisters else cdepth

        return _depth(node, kind)

    def length(self, node):
        return len(self.api.E.oslots.s(node))
    ...
    def getSlots(self, node, kind):
        ...
        return functools.reduce(
            lambda x, y: x | y,
            (set(Eoslotss(leaf)) for leaf in self.getLeaves(node, kind)),
            set(),
        )
    ...
    def getRoot(self, node, kind):
        ...
        elderSister = self.elderSister
        if node in parent:
            return self.getRoot(parent[node], kind)
        if kind == "r":
            if node in elderSister:
                return self.getRoot(elderSister[node], kind)
        return (node, otype)
